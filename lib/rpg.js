...
class RPG {
  ...
  async getUniverseByID (id, extra = false) {
    ...
    const output = Object.assign({}, universe, {
      id: entity.id,
      _id: id,
      link: `https://${config.authority}/universes/${universe.slug}`,
  ...
  async getCharacterByID (id) {
    ...
    let character = result[0][0];
    const universe = await this.getUniverseByID(character['roleplay']);
    character['link'] = `https://${config.authority}/universes/${universe['url']}/characters/${character['url']}`;
    return character;
  }
  ...
  async getVehicles () {
    const result = await this.pool.query(`SELECT * FROM rpg_vehicles`);
    if (!result || !result[0]) return null;
    return result[0];
  }
  ...
  async updateUniverse (universeID, data) {
    if (!data) return false;
    if (!data.channel) return false;
    let sql = `UPDATE rpg_roleplays SET matrixChannelID = "${mysql_real_escape_string(data.channel)}" WHERE id = ${universeID}`;
    let result = await this.pool.query(sql);
    return result;
  }
